...
export const getThemeSelector = (categoryId: string, subcategoryId: string, themeId: string) =>
  ({ categories }: IState): ITheme | undefined =>
    categories.items.find(x => x.id === categoryId)
    ?.subcategories.find(x => x.id === subcategoryId)
    ?.themes.find(x => x.id === themeId);
export const getLoading = ({ categories }: IState): boolean => categories.isLoading;
export const getActiveTheme = ({ categories }: IState): ITheme | null => categories.activeTheme;
